...
st.markdown("""
    ...
    .info-box { border: 1px solid #00ff00; padding: 12px; background: rgba(0, 255, 0, 0.02); font-size: 11px; text-align: left; border-radius: 5px; }
    </style>
...
if not st.session_state['auth']:
    ...
    with col_l:
        ...
        st.markdown("<div class='info-box'>TARGET: ESTABLISHED<br>FREQ: 5.8 GHz<br>SIGNAL: 96%<br>DECRYPTOR: ACTIVE</div>", unsafe_allow_html=True)
    ...
    with col_r:
        ...
        st.markdown("<div class='info-box'>PAYLOAD: READY<br>PROXY: ENABLED<br>LATENCY: 9ms<br>SEC_BYPASS: TRUE</div>", unsafe_allow_html=True)
else:
    # --- 3. القائمة الجانبية ---
    with st.sidebar:
    ...
    # --- 4. النظام الرئيسي (إعادة البناء المطابقة) ---
    ...
    if uploaded_file is not None:
        ...
        with col2:
            ...
            with st.spinner("PROCESSING TEXTURES..."):
                # معالجة حقيقية للصورة الأصلية لتبدو واقعية لنفس الشخصية
                detail = cv2.detailEnhance(img, sigma_s=15, sigma_r=0.15)
